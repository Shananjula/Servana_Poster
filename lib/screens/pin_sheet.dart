...
//
// PIN Sheet — Start/Finish with 4-digit code
  ...
  Future<void> _commit() async {
    ...
    try {
      ...
      if (_expected != null && _expected!.isNotEmpty && _expected != pin) {
        Analytics.log('pin_incorrect', params: {'taskId': widget.taskId, 'mode': widget.mode.name}); throw Exception('Incorrect PIN');
      }
      if (widget.mode == PinMode.start) {
        await ref.set({
          'status': 'ongoing',
          'startedAt': now,
          if (_expected == null || _expected!.isEmpty) 'pinStartEntered': pin,
          'updatedAt': now,
        ...
        await ref.set({
          'status': 'completed',
          'finishedAt': now,
          if (_expected == null || _expected!.isEmpty) 'pinFinishEntered': pin,
          'updatedAt': now,
        }, SetOptions(merge: true));
      }
      if (!mounted) return;
      Analytics.log(widget.mode == PinMode.start ? 'pin_start_success' : 'pin_finish_success', params: {'taskId': widget.taskId});
              ...
              children: [
                Expanded(
                  child: FilledButton.icon(
                    icon: _busy ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.check_circle_rounded),
